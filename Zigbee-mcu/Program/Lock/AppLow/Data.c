...
{
  uint8_t DataResult = DATA_OK;                                                 //数据操作标识
...
{
  uint8_t NullCardRom[4] = {'=','=','=','='};                                   //普通卡空内存标识
...
uint8_t Data_CommonCard_Auth(uint8_t *CardId)
{
  ...
  uint8_t ReadData[4]   = {0x00};
  uint8_t NullRom[4]    = {'=','=','=','='};                                    //空内存标识
  uint8_t UsedRom[4]    = {'-','-','-','-'};                                    //使用过的内存标识
  uint16_t NullPosition = COMMONCARD_MAX1_ADDR;                                 //空内存位置
  uint16_t UsedPosition = COMMONCARD_MAX1_ADDR;                                 //使用过内存
  uint16_t i;
